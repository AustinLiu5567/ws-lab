...
import { Search, ArrowUpRight, Plus, Layers3 } from "lucide-react";
...
export function ModLibrary({ entries }: { entries: ModEntry[] }) {
  ...
  const items = entries.filter(
    (m) =>
      (status === "all" || m.usage_status === status) &&
      (source === "all" || m.origin === source) &&
      [m.title, m.title_en, m.author, m.summary, m.summary_en, m.mod_code]
        .join(" ")
        .toLowerCase()
  ...
  return (
    <>
      <div className="mod-controls mod-catalog-notice">
        <Link className="button primary" href="/submit/mod">
          <Plus size={17} />
          {l("提交 Mod", "Submit a mod")}
        </Link>
        <Link className="button" href="/submissions">
          {l("管理我的作品", "Manage my creations")}
        </Link>
        <p className="bilingual-note">
          {l(
            "资料收录 ≠ 当前版本实测。请查看状态、依赖与来源。",
      ...
      <div className="library-layout">
        <aside className="library-sidebar">
          ...
          <Select value={source} onValueChange={setSource}>
            <SelectTrigger aria-label={l("资料来源", "Catalog source")}>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {[
          ...
          <p>
            {l(
              "暂停、弃用项目保留为历史档案；GitHub 项目独立收录，不代表已加入斯大林格勒。",
        ...
        <section className="library-main">
          <div className="library-top">
            <div className="search-input">
              ...
              <Input
                ...
                aria-label={l("搜索 Mod", "Search mods")}
                placeholder={l(
                  "搜索名称、功能、作者或 Mod 码…",
              ...
              <article className="mod-card" key={m.id}>
                ...
                {m.editable && (
                  <div className="mod-manage-actions">
                    <Link className="text-link" href={"/mods/" + m.id + "/edit"}>
          ...
          {!items.length && (
            <div className="empty-surface">
              <Layers3 size={30} />
              <h2>{l("没有匹配的 Mod", "No matching mods")}</h2>
              <button
                className="button"
                onClick={() => {
                  setQuery("");
          ...
          <div className="workbench-footnote">
            <p>
              {l("想调整基础单位？", "Want to rebalance units?")}{" "}
              <Link className="text-link" href="/workbench">
                {l("打开 Mod 工作台 →", "Open the Mod workbench →")}
              </Link>
            </p>
          </div>
        </section>
