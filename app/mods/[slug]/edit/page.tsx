...
export default async function EditMod({params}:{params:Promise<{slug:string}>}){
 ...
 const content=failed?<p className="notice error"><Bilingual zh="暂时无法加载编辑内容，请稍后重试。" en="The editor could not be loaded. Please try again later."/></p>:entry?<ModEditor initial={entry} admin={admin}/>:<p className="notice error"><Bilingual zh="没有可编辑的 Mod，或你没有此作品的编辑权限。" en="This mod is unavailable, or you do not have permission to edit it."/></p>;
 return <><AtlasHeader/><main className="shell"><div className="chinese-only"><PageTitle eyebrow="创作中心 / MOD 编辑" title="编辑 Mod 档案" text="维护分享码、使用说明与文件，让玩家看到可靠的当前版本。"/></div><div className="english-only"><PageTitle eyebrow="CREATOR / MOD EDITOR" title="Edit mod dossier" text="Maintain codes, instructions, and files so players can find a reliable current version."/></div>{content}</main><AtlasFooter/></>;
}
