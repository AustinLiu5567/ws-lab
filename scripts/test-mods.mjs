...
const seedIds = [
  "stalingrad-rules", "ground-balance", "air-logistics", "lean-economy",
  "late-game", "legendary-heroes", "housing", "strongpoints", "transport",
  "opening-guide", "old-territory", "general-winter", "order-227",
  "old-paratroopers", "paratrooper-test", "paratrooper-diagnostics",
  "external-config", "github-diplomacy", "github-wave-winter",
  "github-nuclear-bomb", "github-adjust-resources", "github-economy-gather",
  "github-wsunitstats"
];
const syntheticBody = {
  ...
  summary_en: "Synthetic local-only metadata for API validation; not a playable or game-tested mod.",
  description: "This local test exercises metadata, moderation, private storage, and revisions. It is not gameplay content. The file contains comments only and is never executed.",
  description_en: "This local test exercises metadata, moderation, private storage, and revisions. It is not gameplay content. The file contains comments only and is never executed.",
  instructions: "Local API test only. Do not install, execute, or publish this synthetic fixture.",
  instructions_en: "Local API test only. Do not install, execute, or publish this synthetic fixture.",
...
async function request(path, {
  method = "GET", auth = false, body, id,
  originHeader = origin, headers = {}
} = {}) {
  ...
  const response = await fetch(url, {
    method, headers: requestHeaders, body, redirect: "error",
    signal: AbortSignal.timeout(30000)
  });
  const bytes = new Uint8Array(await response.arrayBuffer());
  const text = new TextDecoder().decode(bytes);
  let data = null;
  try { data = JSON.parse(text); } catch { /* File responses are not JSON. */ }
...
function review(id, value, auth = true) {
  return request("/api/mods/review/" + id, {
    method: "PATCH", auth, headers: { "Content-Type": "application/json" },
...
try {
  ...
  check("all 23 curated seed IDs are present", seedIds.filter(id => initial.data.items.some(item => item.id === id)).length, 23);
  check("17 Stalingrad seed entries", initial.data.items.filter(item => item.origin === "stalingrad").length, 17);
  check("6 GitHub seed entries", initial.data.items.filter(item => item.origin === "github").length, 6);
  check("public list is approved only", initial.data.items.every(item => item.status === "approved"), true);
  ...
  check("local cookie identifies an administrator", { status: me.status, signedIn: me.data?.signedIn, admin: me.data?.admin }, { status: 200, signedIn: true, admin: true });
  check("anonymous management list denied", (await request("/api/mods/submissions")).status, 401);
  check("anonymous admin management list denied", (await request("/api/mods/submissions?scope=admin")).status, 401);
  originalCurated = await entry(curatedId);
  check("curated entry has no fixture file to replace", originalCurated.has_file, false);

  check("anonymous create denied", (await request("/api/mods", { method: "POST", id: communityId, body: form() })).status, 401);
  check("anonymous curated edit denied", (await request("/api/mods/" + curatedId, { method: "PATCH", body: form(originalCurated) })).status, 401);
  check("cross-origin create denied", (await request("/api/mods", { method: "POST", auth: true, id: communityId, body: form(), originHeader: "https://attacker.invalid" })).status, 403);
  check("missing Origin create denied", (await request("/api/mods", { method: "POST", auth: true, id: communityId, body: form(), originHeader: null })).status, 403);
  check("cross-origin curated edit denied", (await request("/api/mods/" + curatedId, { method: "PATCH", auth: true, body: form(originalCurated), originHeader: "https://attacker.invalid" })).status, 403);
  check("UUID header required", (await request("/api/mods", { method: "POST", auth: true, id: "not-a-uuid", body: form() })).status, 400);
  check("multipart required", (await request("/api/mods", { method: "POST", auth: true, id: communityId, headers: { "Content-Type": "application/json" }, body: "{}" })).status, 400);
  check("upload rights confirmation required", (await request("/api/mods", { method: "POST", auth: true, id: communityId, body: form(syntheticBody, { rights: false }) })).status, 400);
  for (const source_url of ["javascript:alert(1)", "https://user:password@example.invalid/file"]) {
    check("unsafe source URL rejected: " + new URL(source_url).protocol, (await request("/api/mods", { method: "POST", auth: true, id: communityId, body: form({ ...syntheticBody, source_url }) })).status, 400);
  }
  check("malformed Mod code rejected", (await request("/api/mods", { method: "POST", auth: true, id: communityId, body: form({ ...syntheticBody, mod_code: "not-a-valid-code" }) })).status, 400);
  check("source, code, or file required", (await request("/api/mods", { method: "POST", auth: true, id: communityId, body: form({ ...syntheticBody, source_url: "" }) })).status, 400);
  ...
  ]) check(name + " rejected", (await request("/api/mods", { method: "POST", auth: true, id: communityId, body: form(syntheticBody, { file }) })).status, 400);
  ...
  const created = await request("/api/mods", { method: "POST", auth: true, id: communityId, body: createForm });
  ...
  check("fileless submission has no download", (await request("/api/mods/files/" + communityId, { auth: true })).status, 404);
  const retried = await request("/api/mods", { method: "POST", auth: true, id: communityId, body: form({ ...syntheticBody, title: "Retry must not overwrite metadata" }) });
  check("UUID retry is idempotent", retried.status, 200);
  check("UUID retry preserves original title and revision", { title: retried.data.item.title, revision: retried.data.item.revision }, { title: syntheticBody.title, revision: 0 });
  ...
  check("pending Lua cannot be downloaded anonymously", (await request("/api/mods/files/" + communityId)).status, 404);
  ...
  const approval = { status: "approved", feedback: "LOCAL SYNTHETIC API CHECKLIST ONLY; gameplay was not tested.", revision: pending.revision, checks: fullChecks };
  check("anonymous review denied", (await review(communityId, approval, false)).status, 401);
  for (const missing of fullChecks) {
    check("approval requires check " + missing, (await review(communityId, { ...approval, checks: fullChecks.filter(check => check !== missing) })).status, 400);
  }
  check("all four synthetic checks permit review", (await review(communityId, approval)).status, 200);
  ...
  check("download is attachment", download.response.headers.get("content-disposition")?.startsWith("attachment;"), true);
  check("download contains encoded original filename", download.response.headers.get("content-disposition")?.includes(encodeURIComponent(luaName)), true);
  check("download uses octet-stream", download.response.headers.get("content-type"), "application/octet-stream");
  check("download prevents MIME sniffing", download.response.headers.get("x-content-type-options"), "nosniff");
  check("download is not cached", download.response.headers.get("cache-control"), "private, no-store");
  check("download CSP is sandboxed", download.response.headers.get("content-security-policy"), "default-src 'none'; sandbox");
  check("downloaded bytes match checksum", createHash("sha256").update(download.bytes).digest("hex"), luaHash);
  check("stale reviewer revision rejected", (await review(communityId, { ...approval, status: "rejected", feedback: "Local stale-revision test; must not take effect." })).status, 409);

  curatedMutationAttempted = true;
  const curatedChange = { ...originalCurated, summary: originalCurated.summary + " [Local API QA " + communityId.slice(0, 8) + "]" };
  const curatedSaved = await request("/api/mods/" + curatedId, { method: "PATCH", auth: true, body: form(curatedChange) });
  ...
  check("curated database override is public", (await entry(curatedId, false)).summary, curatedChange.summary);
  check("stale curated edit rejected", (await request("/api/mods/" + curatedId, { method: "PATCH", auth: true, body: form(curatedChange) })).status, 409);
  check("curated record cannot use community withdrawal", (await withdraw(curatedId, curatedSaved.data.item.revision)).status, 400);

  const beforeEdit = await entry(communityId);
  const editedBody = { ...beforeEdit, summary: "Edited synthetic local metadata; publication must wait for a new review." };
  const edited = await request("/api/mods/" + communityId, { method: "PATCH", auth: true, body: form(editedBody) });
  ...
  check("edited download is private again", (await request("/api/mods/files/" + communityId)).status, 404);
  check("stale community edit rejected", (await request("/api/mods/" + communityId, { method: "PATCH", auth: true, body: form(editedBody) })).status, 409);
  check("anonymous withdrawal denied", (await withdraw(communityId, edited.data.item.revision, false)).status, 401);
  check("stale withdrawal rejected", (await withdraw(communityId, beforeEdit.revision)).status, 409);
  check("current revision can be withdrawn", (await withdraw(communityId, edited.data.item.revision)).status, 200);
  ...
  check("withdrawn file remains private", (await request("/api/mods/files/" + communityId)).status, 404);
  check("duplicate withdrawal rejected", (await withdraw(communityId, withdrawn.revision)).status, 409);
  check("withdrawn record cannot be approved", (await review(communityId, { ...approval, revision: withdrawn.revision })).status, 404);
  const managed = await request("/api/mods/submissions?scope=admin", { auth: true });
  check("admin management list includes withdrawn audit record", managed.data.items.some(item => item.id === communityId && item.status === "withdrawn"), true);
} catch (error) {
  failure = error;
} finally {
  ...
  if (curatedMutationAttempted && originalCurated) {
    try {
      ...
      cleanup.push("Curated content restored exactly; local revision/audit timestamps may advance.");
    } catch (error) {
  ...
  console.log(JSON.stringify({
    passed: report.length, scope: origin + " (local only)",
    communityRecord: createAttempted ? communityId : null, cleanup,
    notCovered: [
