...
export async function featuredRow() {
  return bindings()
    .DB.prepare("SELECT * FROM featured_maps WHERE id = ?")
    .bind("stalingrad")
    .first<FeaturedRow>();
}
export async function getFeaturedMap(): Promise<EditableMap> {
  const r = await featuredRow();
  if (!r) return stalingradDefault;
  return {
    ...stalingradDefault,
    ...JSON.parse(r.body),
    id: r.id,
