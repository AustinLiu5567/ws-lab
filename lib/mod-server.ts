...
export async function rawMod(id:string):Promise<ModRecord|null> {
  const row = await bindings().DB.prepare("SELECT * FROM community_mods WHERE id = ?")
    .bind(id).first<ModRecord>();
...
export async function listMods(userId?:string, admin?:boolean, manage=false):Promise<ModEntry[]> {
  if (userId === undefined && admin === undefined) {
    const viewer = await identity();
    userId = viewer.user?.userId;
    admin = viewer.admin;
  }
  const result = await bindings().DB.prepare("SELECT * FROM community_mods ORDER BY updated_at DESC, id ASC")
    .all<ModRecord>();
