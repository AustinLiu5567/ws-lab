...
export function generateGameplayLua(
  catalog,
  edits,
  title = "WS ATLAS balance",
  rules = [],
  conflictMode = "abort",
) {
  ...
  for (const edit of edits) {
    const unit = byId.get(edit.unitId),
      field = unit?.fields.find((f) => f.key === edit.fieldKey);
    if (!field) throw new Error("Unknown unit or field");
    const id = unit.id + ":" + field.key,
      value = edit.value,
      raw = Math.round(value * field.scale);
    if (seen.has(id)) throw new Error("Duplicate edit");
    seen.add(id);
    if (
      typeof value !== "number" ||
      ...
      (!field.round && Math.abs(raw - value * field.scale) > 1e-7) ||
      (field.kind === "boolean" && value !== 0 && value !== 1)
    )
      throw new Error("Invalid value: " + id);
    if (!/^root\.(?:unitType|build)\[\d+\](?:\.[A-Za-z][A-Za-z0-9_]*|\[\d+\])+$/.test(field.path))
      throw new Error("Untrusted catalog path");
    if (raw === field.raw) continue;
    if (paths.has(field.path)) {
      if (paths.get(field.path) !== raw)
        throw new Error("The same producer field has conflicting edits.");
      continue;
    }
    paths.set(field.path, raw);
    changed.push({ unit, field, value, raw });
  }
  const value = (u, k, fallback) => {
    const f = u.fields.find((f) => f.key === k);
    return f && paths.has(f.path) ? paths.get(f.path) / f.scale : fallback;
  };
  for (const u of byId.values()) {
    for (const w of u.weapons) {
      ...
      if (
        !keys.some((k) => {
          const f = u.fields.find((x) => x.key === k);
          return f && paths.has(f.path);
        })
      )
        continue;
      const min = value(u, keys[0], w.rangeMin ?? 0),
        max = value(u, keys[1], w.rangeMax),
        stop = value(u, keys[2], w.rangeStop);
      if (max === null || stop === null || min > max || max >= stop)
        throw new Error("Range must satisfy minimum <= maximum < stop");
    }
    if (changed.some((e) => e.unit.id === u.id && e.field.key.startsWith("action-"))) {
      const lo = u.fields.find((f) => f.key === "action-min"),
        hi = u.fields.find((f) => f.key === "action-max");
      if (lo && hi && value(u, lo.key, lo.value) > value(u, hi.key, hi.value))
        throw new Error("Action minimum distance exceeds maximum");
    }
    if (
      changed.some((e) => e.unit.id === u.id && /^armor-\d+-weight$/.test(e.field.key)) &&
      u.armor.reduce(
        (n, a) => n + value(u, "armor-" + a.index + "-weight", a.probabilityWeight ?? 1),
        0,
      ) <= 0
    )
      throw new Error("Armor weights must have a positive sum");
  }
  const ruleSeen = new Set(),
    ruleGroups = new Map();
  const checkedRules = rules.map((rule) => {
    const u = byId.get(rule.unitId),
      target = u?.ruleTargets?.find((t) => t.key === rule.targetKey);
    if (
      !target ||
      !byId.has(rule.requiredUnitId) ||
      ![rule.min, rule.max].every((n) => Number.isInteger(n) && n >= 0 && n <= 65535) ||
      rule.min > rule.max
    )
      throw new Error("Invalid unit requirement");
    if (target.requirements.unitsAll === false)
      throw new Error("Any-of unit requirements are read-only");
    const key = u.id + ":" + target.key + ":" + rule.requiredUnitId;
    if (ruleSeen.has(key)) throw new Error("Duplicate unit requirement");
    ruleSeen.add(key);
    const group = u.id + ":" + target.key,
      ids = ruleGroups.get(group) || new Set(target.requirements.units.map((x) => x.id));
    ids.add(rule.requiredUnitId);
    ruleGroups.set(group, ids);
    if (ids.size > 4) throw new Error("At most four unit conditions per target are supported");
    return {
      ...rule,
      unit: u,
      target,
      baseline: target.requirements.units.find((x) => x.id === rule.requiredUnitId),
    };
  });
  if (!changed.length && !checkedRules.length) throw new Error("No changed values to export");
  const q = (v) =>
    JSON.stringify(String(v).replace(/[\r\n\u0000-\u001f]/g, " ")).replace(
      /\\u([0-9a-f]{4})/gi,
      (_, hex) => String.fromCharCode(parseInt(hex, 16)),
    );
  const tokens = (path) =>
    [...path.slice(5).matchAll(/([A-Za-z][A-Za-z0-9_]*)|\[(\d+)\]/g)]
      .map((m) => (m[1] ? q(m[1]) : m[2]))
      .join(", ");
  const guard = (g) =>
    g
      ? ", guard={producer=" +
        ...
        ", kind=" +
        (g.abilityType ?? 0) +
        (g.targetId !== undefined ? ", target=" + g.targetId : "") +
        (g.researchId !== undefined ? ", research=" + g.researchId : "") +
        "}"
      : "";
  const patches = changed.map(
    ({ unit, field, raw }) =>
      "    {path={" +
      tokens(field.path) +
      "}, value=" +
      (field.kind === "boolean" ? String(!!raw) : raw) +
      ", expected=" +
      (field.kind === "boolean" ? String(!!field.raw) : field.raw) +
      ", label=" +
      q(unit.name + " #" + unit.id + " " + field.label) +
      (field.scope === "faction-build" ? ", build=" + field.buildId + ", unit=" + unit.id : "") +
      guard(field.productionGuard) +
      (field.abilityGuard
        ? ", abilityUnit=" +
          unit.id +
          ", ability=" +
          field.abilityGuard.abilityId +
          ", abilityKind=" +
          field.abilityGuard.type +
          (field.abilityGuard.targetId !== undefined
            ? ", effectTarget=" + field.abilityGuard.targetId
            : "") +
          (field.abilityGuard.researchId !== undefined
            ? ", effectResearch=" + field.abilityGuard.researchId
            : "")
        : "") +
      (field.buildWorkerGuard
        ? ", workerUnit=" +
          unit.id +
          ", workerIndex=" +
          field.buildWorkerGuard.index +
          ", workerTarget=" +
          field.buildWorkerGuard.unitId
        : "") +
      "},",
  );
  const rows = checkedRules.map(
    (r) =>
      "    {path={" +
      tokens(r.target.path) +
      "}, required=" +
      r.requiredUnitId +
      ", min=" +
      r.min +
      ...
      r.max +
      ", label=" +
      q(r.unit.name + " " + r.target.key + " requires #" + r.requiredUnitId) +
      (r.target.buildId !== undefined
        ? ", build=" + r.target.buildId + ", unit=" + r.unit.id
        : "") +
      guard(r.target.guard) +
      (r.baseline ? ", baselineMin=" + r.baseline.min + ", baselineMax=" + r.baseline.max : "") +
      "},",
  );
  const attacks = [
    ...new Set(changed.filter((e) => e.field.path.includes(".attack.")).map((e) => e.unit.id)),
  ];
  return [
    "-- " + String(title).replace(/[\r\n\u0000-\u001f]/g, " "),
    "-- WS ATLAS gameplay Lua. Paste into a gameplay mod and test privately.",
    "-- Baseline: Steam " +
      catalog.provenance.steamBuild +
      "; Gameplay " +
      catalog.provenance.gameplayVersion +
      ".",
