...
const stalingradSeeds: ModEntry[] = modLibrary.map((m) => {
  ...
  return curatedSeed(
    m.slug,
    {
      ...
      instructions:
        "本条目是斯大林格勒资料归档，不含下载文件或已核实的游戏内代码。请核对作者实际挂载版本，先在私人测试局验证。" +
        (paired ? "玩法与视觉脚本必须配套。" : ""),
      instructions_en:
        "This is a Stalingrad archive entry, without a downloadable file or verified in-game code. Check the author's actual loaded version and verify it in a private match first." +
        (paired ? " Pair the gameplay and visual scripts." : ""),
      compatibility:
        m.note +
        "\n\n资料日期：" +
        m.asOf +
        "。保留方案不等于已实机验收；暂停、弃用及诊断项不要直接加入正式对局。",
      compatibility_en:
        english.note +
        "\n\nArchive date: " +
        m.asOf +
        ". Retained does not mean verified in-game. Do not add paused, retired, or diagnostic entries directly to regular matches.",
      mod_code: "",
      map_slug: "stalingrad",
...
const githubBodies: (ModBody & { id: string })[] = [
  {
    ...
    description:
      "这是完整的玩家外交组合。选择操作后点击对方领土，即可发出提议或宣战；结盟与和平需要对方回应，宣战立即生效。所有存活玩家互为盟友时可共同获胜。默认至少三名玩家存活才能结盟，每人最多五名盟友。\n\n原作者：ShiJueXiangGuan、UIXlangGuan、WaiJiaoMod；英文翻译与重发布：JSuisMort。",
    description_en:
      "A coordinated diplomacy bundle: choose an action, then target another player's territory. Alliance and peace require a response; war takes effect immediately. Surviving players can win together when all are mutual allies. Defaults require at least three living players to form alliances and allow five allies per player.\n\nOriginal authors: ShiJueXiangGuan, UIXlangGuan, WaiJiaoMod. English translation and republication: JSuisMort.",
    instructions:
      "在地图编辑器打开地图，进入 Mods → Add a modification，逐个添加所需代码，然后保存并发布地图。组合项目须遵循各条目的依赖说明。\n\n必须同时添加三个代码。英文界面；与复用同一原生界面节点的 Mod 可能冲突。",
    instructions_en:
      "Open the map in the editor, choose Mods → Add a modification, add the required codes, then save and publish the map. Follow each bundle's dependency notes.\n\nInstall all three codes. English interface; mods reusing the same native UI nodes may conflict.",
    compatibility:
    ...
    game_version: "",
    mod_code:
      "Diplomacy interface: mod-s2u4EUGfise\nGameplay backend: mod-HmXZrJBjwM6\nUI framework: mod-KbwSsR2og7a",
    source_url:
      "https://github.com/AdrienRmd/War_Selection_Modding/blob/main/mods/diplomacy/README.md",
  ...
  },
  {
    ...
    description:
      "波次系统让专用 AI 阵营持续进攻玩家基地，随阶段与存活人数调整压力，并支持奇观胜利。寒冬部分降低移动和视野，配合冷蓝光照。可单用 PVE，或使用寒冬双脚本，或全部组合；不是无需配置即可运行的完整地图。\n\nAustin 与 Nuanyang 原作；AdrienRmd 英文化。",
    description_en:
      "Wave PVE sends a dedicated AI faction against player bases, scaling pressure by progression and survivors, with wonder victory support. Winter adds movement and vision penalties with cold-blue lighting. Use PVE alone, the Winter pair, or the full bundle; this is not a preconfigured standalone map.\n\nOriginal work by Austin and Nuanyang; English texts by AdrienRmd.",
    instructions:
  ...
  },
  {
    ...
    description:
      "在后期科技后开放飞机核弹，并为间谍核弹设置独立科技门槛。设置面板可调整食物、木材、铁的成本，以及爆炸范围和伤害。每架飞机每局只能投放一枚，适合设计高代价的后期打击规则。",
    description_en:
      "Adds technology-gated aircraft nukes and a separate research requirement for the spy's bomb. The settings panel controls food, wood and iron costs, blast radius, and damage. Each aircraft can drop only one bomb per game, creating a costly late-game strike option.",
    instructions:
    ...
    source_url:
      "https://github.com/AdrienRmd/War_Selection_Modding/blob/main/mods/nuclear_bomb/README.md",
    license: "MIT (repository-level declaration)",
    map_slug: "",
    kind: "gameplay",
    usage_status: "unverified",
  },
  {
    ...
    description:
      "开局时统一重设浆果、鱼群、小麦、石矿和铁矿的单点资源量；树木使用百分比缩放储量。它改变地图经济的起始条件，不是玩家初始国库设置，也不提供资源再生。\n\nAustin 原作，AdrienRmd 修改英文文本。",
    description_en:
      "At match start, sets per-node quantities for berries, fish, wheat, stone, and iron, while scaling tree wood by a percentage. It changes the map's resource supply, not the player's starting treasury, and does not regenerate exhausted resources.\n\nOriginal by Austin; English-text modifications by AdrienRmd.",
    instructions:
    ...
    source_url:
      "https://github.com/AdrienRmd/War_Selection_Modding/blob/main/mods/adjust_resources/README.md",
    license: "MIT (repository-level declaration)",
    map_slug: "",
    kind: "gameplay",
    usage_status: "unverified",
  },
  {
    ...
    description:
      "六个独立模块覆盖工人与渔船的采集／携带能力，以及多类建筑的储量比例。地图作者可仅选需要的部分，而不必整套启用。除 Worker 需要直接编辑脚本外，其余五项通过设置面板调整；默认数值另有参考表。",
    description_en:
      "Six independent modules cover worker and fishing-boat gathering/carrying plus storage multipliers for several building groups. Map makers can enable only the components they need. Five modules use settings panels; Worker is configured in its script. A separate defaults reference supports baseline comparisons.",
    instructions:
  ...
  },
  {
    ...
    description:
      "这是 wsunitstats.com 的源码项目，不是可安装玩法 Mod。网站提供单位和科技数据、研究后的属性比较及引擎数据浏览；Java 导出器从游戏数据生成网站所需内容。可作为设计与核对参数的参考入口。",
    description_en:
      "The source project behind wsunitstats.com, not an installable gameplay mod. Its website covers unit/research statistics, research-adjusted values, and engine-data browsing. A Java exporter prepares data for the React interface. Use it as a reference when inspecting or designing mod parameters.",
    instructions:
      "参考网站：https://wsunitstats.com\n普通读者可访问网站；自行构建导出器需要 Java 17 与 Maven。没有已核实的游戏内 Mod 代码。",
    instructions_en:
      "Reference website: https://wsunitstats.com\nReaders can use the website. Building the exporter requires Java 17 and Maven. No in-game Mod code was verified.",
    compatibility: "已查看的仓库根目录与 README 未列出明确许可证；不据此推断代码或素材可再分发。",
    compatibility_en:
      "No explicit license appeared in the inspected repository root or README; do not infer redistribution rights for code or assets.",
    game_version: "",
    mod_code: "",
